...
- 이 repo는 SDF 기반 neural surface reconstruction을 수학적으로 이해하기 위한 자료입니다. 최근 primitive-based 3DGS 계열과 비교할 때, SDF/volume rendering 계열은 surface normal, eikonal regularization, occlusion-aware weight가 강점입니다.
- origin/main의 PyTorch Colab 노트북까지 fast-forward 반영했습니다. 로컬에는 `.claude/settings.local.json` 수정이 남아 있는데, 사용자 설정 파일로 보여 커밋 대상에서 제외했습니다.
- 3dgrut/neural-harmonic-textures와 같이 읽으면 "implicit surface", "primitive-based representation", "neural texture/feature scaffold"가 서로 어떤 장단점을 가지는지 비교할 수 있습니다.
